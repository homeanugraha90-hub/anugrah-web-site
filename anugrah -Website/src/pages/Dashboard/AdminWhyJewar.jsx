...
export default function AdminWhyJewar() {
  ...
  const fetchSections = async () => {
    try {
      const res = await axios.get("http://localhost:5000/api/why-jeware");
  ...
  const handleSelect = (section) => {
    ...
    if (section.description) {
      try {
        setContent(JSON.parse(section.description));
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedSection.sectionName === "ThirdSection") {
      ...
      await axios.put(
        `http://localhost:5000/api/why-jeware/${selectedSection.sectionName}`,
        formData,
