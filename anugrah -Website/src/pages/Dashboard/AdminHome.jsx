...
export default function AdminHome() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (selectedSection.videos && selectedSection.videos.length > 0) {
                formData.append("existingVideos", JSON.stringify(selectedSection.videos));
            }

            await axios.put(
                `http://localhost:5000/api/home/${selectedSection.sectionName}`,
                formData,
            ...
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="md:w-1/4 bg-white rounded-lg shadow p-4">
                    ...
                    <ul className="space-y-2">
                        {sections.map((section) => (
                            <li key={section._id}>
                                <button
                                    className={`w-full text-left px-3 py-2 rounded hover:bg-green-100 transition ${selectedSection?._id === section._id
                ...
                {selectedSection && (
                    <div className="md:w-3/4 bg-white rounded-lg shadow p-6 flex flex-col gap-4">
                        ...
                        {content && Object.keys(content).length > 0 ? (
                            Object.keys(content).map((key) => (
                                <div key={key} className="flex flex-col">
                                    ...
                                    <textarea
                                        className="border rounded-md p-2 resize-none focus:outline-none focus:ring-2 focus:ring-green-400"
                                        rows={key.includes("discribe") ? 3 : 1}
                                        value={content[key] || ""}
                                        onChange={(e) =>
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {selectedSection.images.map((img, idx) => (
                                        <div key={idx} className="relative">
                                            <img
                                                src={`http://localhost:5000/upload/${img}`} // use the correct path
                                                alt={`img-${idx}`}
                        ...
                        {/* Existing Video */}
                        {selectedSection.videos && selectedSection.videos.length > 0 && (
                            <div>
                                <label className="font-medium text-gray-700 mb-1">Existing Video</label>
                                <div className="relative w-64 h-40">
                                    <video
                                        src={`http://localhost:5000/uploads/${selectedSection.videos[0]}`}
                                        controls
                        ...
                        <button
                            onClick={handleSave}
                            disabled={loading}
                            className="self-start mt-4 bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded-lg flex items-center gap-2 transition"
                        >
